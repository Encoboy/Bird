	...
	*************************/
	...
	function createPipe(){
		...
		upPipe.moveTimer = setInterval(function(){
			...
			// 红
			/*当管子向左滚出游戏界面，管子又回到最开始的位置，又开始重新计算管子的高度*/
			if(l <= -w-2 ){
				/*计算管子高度的函数*/
				pipHeight();
				/*回到最开始的位置*/
			...
			/*小鸟不能超过的上方的距离，修改小鸟飞出游戏界面的bug*/
			if(upPipe.addScore &&bt<=8){
				oBird.offsetTop=0;
				gameover();
			}
			/*如果小鸟满足了if条件，小鸟就会死，碰撞检测*/
			...
			if(upPipe.addPipe && l <= 140.5){
				createPipe();
				/*当upPipe.addPipe=false时，就不会创建管子，这时已经有了两个管子*/
				upPipe.addPipe = false;
			}
			// 通过后加分
			if(upPipe.addScore && l + w < bl){
				/*就是为了解决管子通过小鸟后会加很多分的情况*/
				upPipe.addScore = false;
				score++;
				var htmlStr = '';
				/*toString可以把一个逻辑值转换成字符串*/
				var scoreStr = score.toString();
				// split 可以用不同的方式分割成字符串
				var scoreArray = scoreStr.split('');
				/* map()方法返回一个由原数组中的每个元素调用一个指定方法后的返回值组成的新数组,就是原来数组
				中的每一个元素都会经过map()里的一个函数运算，得到一个新的数组，是破环性操作*/
				var imgArray = scoreArray.map(function(item){
					return "<img src = 'img/"+item+".jpg'>";
				});
				// jion 用于把数组中的所有元素放入一个字符串
